...
complete -c ec_probe -r -s e -l embedded-controller -d 'Specify embedded controller to use' -f -a 'ec_linux ec_sys_linux'
...
complete -c ec_probe -r -n '__fish_seen_subcommand_from read' -n 'test (__fish_number_of_cmd_args_wo_opts) = 2' -d 'Register source' -f -a '(seq 0 255)'
complete -c ec_probe -n '__fish_seen_subcommand_from write' -s h -l help -d 'show this help message and exit'
complete -c ec_probe -r -n '__fish_seen_subcommand_from write' -n 'test (__fish_number_of_cmd_args_wo_opts) = 2' -d 'Register destination' -f -a '(seq 0 255)'
complete -c ec_probe -r -n '__fish_seen_subcommand_from write' -n 'test (__fish_number_of_cmd_args_wo_opts) = 3' -d 'Value to write' -f -a '(seq 0 255)'
